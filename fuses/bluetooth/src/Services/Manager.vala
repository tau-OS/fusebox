...
public class Bluetooth.Services.ObjectManager : Object {
    ...
    public string ? get_name () {
        ...
        if (adapters.is_empty ()) {
            ...
            return adapters.first ().data.name;
        }
    }

    public GLib.List<Bluetooth.Services.Adapter> get_adapters () {
        var adapters = new GLib.List<Bluetooth.Services.Adapter> ();
        if (object_manager != null) {
            object_manager.get_objects ().foreach ((object) => {
                ...
                adapters.insert (((Bluetooth.Services.Adapter) iface), 0);
            });
        }
    ...
    public GLib.List<Bluetooth.Services.Device> get_devices () {
        var devices = new GLib.List<Bluetooth.Services.Device> ();
        if (object_manager != null) {
            object_manager.get_objects ().foreach ((object) => {
                ...
                devices.insert (((Bluetooth.Services.Device) iface), 0);
            });
        }
